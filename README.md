...
**"모든 앱에서 동작"은 보장하지 않습니다.** macOS 앱마다 입력창의 접근성 구현이 다르고, 비밀번호·보안 입력창은 의도적으로 변환을 차단합니다. 아래는 실제 물리 키 이벤트를 넣어 확인한 결과입니다.
...
- ad-hoc 서명은 지정 요구사항이 cdhash에 고정되므로, 나중에 새 버전으로 앱을 교체하면 손쉬운 사용·입력 모니터링 승인이 폐기됩니다. 기존 항목을 `−`로 제거한 뒤 `+`로 다시 추가해야 하며, 토글만 껐다 켜는 것으로는 해결되지 않습니다.
